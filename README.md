...
Each pattern corresponds to different spirals created by the 2 degree of freedom robotic arm, which uses inverse kinematics for its motion. To improve aesthetics of the project, we also incorporated an external LED strip which is plugged into a USB port, although this part is not directly connected to the Arduino at all. 
